...
class FirebaseService {
  ...
  /// Kullanıcının profil bilgilerini (Yaş, Boy, Kilo vb.) veritabanına kaydeder.
  ...
  /// KULLANICININ TÜM VERİLERİNİ SİLER (RESET)
  ...
  Future<void> addWaterLog(String userId, double amount) async {
    ...
    if (amount > 0) {
      await dayRef.collection('logs').add({
        'amount': amount,
  ...
  Future<int> getStreak(String userId) async {
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        return (doc.data() as Map<String, dynamic>)['streak'] ?? 0;
      }
      return 0;
    } catch (e) { return 0; }
  }
  ...
  Future<void> updateStreak(String userId, int newStreak) async {
    ...
    await _firestore.collection('users').doc(userId).set({
      'streak': newStreak,
      'lastStreakDate': DateFormat('yyyy-MM-dd').format(DateTime.now()),
    }, SetOptions(merge: true));
  }
  ...
  Future<Map<DateTime, int>> getHeatmapData(String userId) async {
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        int amount = (doc['amount'] as num).toInt();
        heatmapData[cleanDate] = amount;
      }
      return heatmapData;
    } catch (e) { return {}; }
  }
    ...
    for (int i = 0; i <= 7; i++) {
      ...
      try {
        ...
        for (var doc in logSnap.docs) {
          timeLogs.add((doc['timestamp'] as Timestamp).toDate());
        }
      } catch (e) { continue; }
    }
    return timeLogs;
  ...
  Future<List<Map<String, dynamic>>> getWeeklyData(String userId) async {
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        double val = (doc.data() as Map<String, dynamic>)['amount']?.toDouble() ?? 0;
        rawData[dateKey] = val;
      }
      ...
      for (int i = 6; i >= 0; i--) {
        DateTime day = now.subtract(Duration(days: i));
        String dayKey = DateFormat('yyyy-MM-dd').format(day);
        String label = DateFormat('E', 'tr').format(day);
        formattedList.add({'label': label, 'val': rawData[dayKey] ?? 0.0});
      }
      return formattedList;
    } catch (e) {
      return List.generate(7, (index) => {'label': '-', 'val': 0.0});
    }
  }
}