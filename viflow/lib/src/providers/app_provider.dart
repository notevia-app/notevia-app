...
import 'package:viflow/src/core/services/auth_service.dart'; // Auth servisi
...
class AppProvider extends ChangeNotifier {
  ...
  double _targetWater = 2500;
  String _userName = "";
  String _userId = ""; // Firebase Auth UID
  int _streak = 0;
  ...
  int _startHour = 9; int _startMinute = 0; int _endHour = 22; int _endMinute = 0; int _frequencyMinutes = 45;
  ...
  // 2. GETTERS
  // ==================================================
  double get currentWater => _currentWater;
  double get targetWater => _targetWater;
  ...
  int get activityIndex => _activityIndex;
  ...
  TimeOfDay get endTime => TimeOfDay(hour: _endHour, minute: _endMinute);
  ...
  final List<String> _tips = [ "Harikasın! Günün yarısı bitti.", "Unutma: Baş ağrısının en büyük sebebi susuzluktur.", "Cildin için bir bardak daha içmeye ne dersin?", "Su içmek metabolizmanı hızlandırır." ];

  // ==================================================
  // 3. BİLİMSEL HESAPLAMA
  // ==================================================
  double _calculateScientificGoal({required double weight, required String gender, required int activityIdx}) {
    double baseGoal = weight * 33;
    if (gender == 'male') baseGoal *= 1.05;
    double activityAddon = 0;
    if (activityIdx == 1) activityAddon = 350; if (activityIdx == 2) activityAddon = 750;
    double totalGoal = baseGoal + activityAddon;
    if (totalGoal < 1500) totalGoal = 1500; if (totalGoal > 4500) totalGoal = 4500;
    return (totalGoal / 10).round() * 10.0;
  }

  // ==================================================
  // 4. BAŞLANGIÇ (LOAD DATA)
  // ==================================================
  Future<void> loadData() async {
    final prefs = await SharedPreferences.getInstance();

    // --- KİMLİK YÖNETİMİ (Firebase Auth) ---
    String? secureId = await AuthService().signInAnonymously();

    if (secureId != null) {
      _userId = secureId;
      await prefs.setString('userId', _userId); // Yedek ID
    } else {
  ...
  Future<void> completeOnboarding({
    required String name, required int age, required double weight,
    required double height, required String gender, required int activityIdx,
  }) async {
    final prefs = await SharedPreferences.getInstance();

    double calculatedTarget = _calculateScientificGoal(weight: weight, gender: gender, activityIdx: activityIdx);

    _userName = name; _targetWater = calculatedTarget; _age = age; _weight = weight;
    _height = height; _gender = gender; _activityIndex = activityIdx; _isFirstTime = false;

    // Yerel Kayıt
    await prefs.setString('userName', name);
    await prefs.setDouble('targetWater', calculatedTarget);
    await prefs.setInt('age', age);
    await prefs.setDouble('weight', weight);
    await prefs.setDouble('height', height);
    await prefs.setString('gender', gender);
    await prefs.setInt('activityIndex', activityIdx);
    await prefs.setBool('isFirstTime', false);

    // Firebase Kayıt
    await FirebaseService().saveUserData(
        userId: _userId, name: name, age: age, weight: weight, height: height,
        gender: gender, activityIndex: activityIdx, dailyGoal: calculatedTarget
    );
  ...
  Future<void> updateUserProfile({
    ...
    _userName = name; _age = age; _weight = weight; _height = height;
    _gender = gender; _activityIndex = activityIdx; _targetWater = newTarget;
    ...
    await prefs.setString('gender', gender);
    await prefs.setInt('activityIndex', activityIdx);
    await prefs.setDouble('targetWater', newTarget);

    // Firebase Kayıt
    await FirebaseService().saveUserData(
        userId: _userId, name: name, age: age, weight: weight, height: height,
        gender: gender, activityIndex: activityIdx, dailyGoal: newTarget
    );
  ...
  // ==================================================
  // 6. SU EKLEME VE STREAK
  // ==================================================
  Future<void> addWater(double amount) async {
    _currentWater += amount; if (_currentWater < 0) _currentWater = 0;

    if (amount > 0) {
      _todayLogs.insert(0, <String, dynamic>{'amount': amount, 'timestamp': DateTime.now()});
    } else {
      if (_todayLogs.isNotEmpty) _todayLogs.removeAt(0);
    }

    notifyListeners();

    final prefs = await SharedPreferences.getInstance();
    await prefs.setDouble('currentWater', _currentWater);

    try { await FirebaseService().addWaterLog(_userId, amount); }
  ...
  Future<void> _checkAndUpdateStreak(SharedPreferences prefs) async {
    ...
    if (_currentWater >= _targetWater && lastStreakDate != today) {
      _streak++;
      await prefs.setString('lastStreakUpdate', today);
      await prefs.setInt('streak', _streak);
      try { await FirebaseService().updateStreak(_userId, _streak); } catch (e) {}
      notifyListeners();
    }
  }
  ...
  Future<void> updateReminderSettings({TimeOfDay? start, TimeOfDay? end, int? freq}) async {
    ...
    await prefs.setInt('startHour', _startHour); await prefs.setInt('startMinute', _startMinute);
    await prefs.setInt('endHour', _endHour); await prefs.setInt('endMinute', _endMinute);
    await prefs.setInt('frequencyMinutes', _frequencyMinutes);
  ...
  Future<void> _scheduleNotifications() async {
    await NotificationService().scheduleDailyNotifications(
        ...
        intervalMinutes: _frequencyMinutes, title: "Viflow", body: "Su içme zamanı! 💧");
  }

  // ==================================================
  // 8. DİL, TEMA VE SIFIRLAMA
  // ==================================================
  ...
  Future<void> setThemeMode(ThemeMode mode) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    int val = 0;
    if (mode == ThemeMode.light) val = 1; if (mode == ThemeMode.dark) val = 2;
    await prefs.setInt('themeMode', val);
    notifyListeners();
  }

  Future<void> updateTarget(double newTarget) async {
    _targetWater = newTarget;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setDouble('targetWater', newTarget);
    notifyListeners();
  }

  // VERİ SIFIRLAMA (BUG FIX İLE)
  Future<void> resetAllData() async {
    final prefs = await SharedPreferences.getInstance();
    String oldUserId = _userId; // Silme işlemi için ID'yi sakla

    // 1. Yerel & Bildirim Temizliği
    await prefs.clear();
    ...
    // 3. Firebase Verilerini Sil
    if (oldUserId.isNotEmpty) {
      await FirebaseService().deleteUserHistory(oldUserId);
    }

    // 4. Provider State'ini Sıfırla
    _currentWater = 0; _targetWater = 2500; _userName = "";
    _isFirstTime = true; _streak = 0; _todayLogs = [];
    _age = 25; _weight = 70; _height = 170; _gender = 'male'; _activityIndex = 1;
    _isReminderActive = false; _userId = ""; // ID'yi temizle

    // 5. UI'ı güncelle (Uygulama Splash'e dönecek)
    notifyListeners();
  }
}